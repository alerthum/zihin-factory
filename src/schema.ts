...
`CREATE TABLE IF NOT EXISTS FACTORY_LESSONS (id TEXT PRIMARY KEY,scope_type TEXT NOT NULL,scope_key TEXT NOT NULL,defect_code TEXT NOT NULL,lesson_text TEXT NOT NULL,severity TEXT NOT NULL DEFAULT 'medium',occurrences INTEGER NOT NULL DEFAULT 1,resolved_successes INTEGER NOT NULL DEFAULT 0,active INTEGER NOT NULL DEFAULT 1,first_seen_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,last_seen_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,last_resolved_at TEXT,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(scope_type,scope_key,defect_code))`,
`CREATE INDEX IF NOT EXISTS IX_FACTORY_LESSONS_ACTIVE ON FACTORY_LESSONS(scope_type,scope_key,active,occurrences DESC)`,
`CREATE TABLE IF NOT EXISTS QUALITY_DEFECTS (id TEXT PRIMARY KEY,job_id TEXT,run_id TEXT,producer_role TEXT,defect_code TEXT NOT NULL,detail TEXT,attempt_no INTEGER,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
...
export async function ensureSchema(db: D1Database): Promise<void> {
  ...
  for (const [key,value] of [["schema_version","12"],["factory_version","0.10.1"]]) {
    await db.prepare(
  ...
  const state = [
    ...
    ["factory_phase","learning-project-director"],
